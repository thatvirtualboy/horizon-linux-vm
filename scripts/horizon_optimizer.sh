...
echo
sleep 2s
read -p "Type your domain controller's IP address: " domaincontrollerip;
echo
sleep 2s
read -p "Type your WINS server IP address: " wins;
echo
sleep 2s
...
case $yn in
Yes ) read -p "Type your primary DNS server and hit [ENTER] " enterdns1; read -p "Type your secondary DNS server and hit [ENTER] " enterdns2;
...
sudo sed -i '14 s/2/5/' /etc/init/rc-sysinit.conf

# Update nsswitch
sudo sed -i '7 s/.*passwd:.*/passwd:         compat winbind/' /etc/nsswitch.conf
sudo sed -i '9 s/.*shadow:.*/shadow:         compat winbind/' /etc/nsswitch.conf
sudo sed -i '11 s/.*hosts:.*/hosts:          cache db files dns/' /etc/nsswitch.conf
...
sudo sed -i '2 s/.*user-session=.*/user-session=gnome-fallback/' /usr/share/lightdm/lightdm.conf.d/50-ubuntu.conf
echo 'greeter-show-manual-login=true' >> /usr/share/lightdm/lightdm.conf.d/50-ubuntu.conf
echo 'greeter-hide-users=true' >> /usr/share/lightdm/lightdm.conf.d/50-ubuntu.conf
echo 'allow-guest=false' >> /usr/share/lightdm/lightdm.conf.d/50-ubuntu.conf
sudo mv /usr/share/xsessions/gnome-fallback-compiz.desktop /usr/share/xsessions/gnome-fallback-compiz.desktop.disable 

# Update Hosts file
sudo echo $domaincontrollerip $domaincontroller'.'$domainname $domaincontroller >> /etc/hosts 
...
# Configure KRB5
sudo sed -i "3 s/.*default_realm.*/default_realm = ${domainname^^}/" /etc/krb5.conf
sudo sed -i "7 s/.*YOURDOMAIN.*/${domainname^^} = {/" /etc/krb5.conf 
sudo sed -i "8 s/.*kdc.*/kdc = ${domaincontrollerip}/" /etc/krb5.conf
sudo sed -i "9 s/.*default_domain.*/default_domain = ${domainname^^}/" /etc/krb5.conf
sudo sed -i "12 s/.*.yourdomain.*/.${domainname,,} = ${domainname^^}/" /etc/krb5.conf
sudo sed -i "13 s/.*yourdomain.*/${domainname,,} = ${domainname^^}/" /etc/krb5.conf


# Configure SMB
sudo sed -i "2 s/.*workgroup.*/workgroup = ${domainname,,}/" /etc/samba/smb.conf
sudo sed -i "3 s/.*password.*/password server = ${domaincontroller,,}.${domainname,,}/" /etc/samba/smb.conf 
sudo sed -i "4 s/.*wins.*/wins server = $wins/" /etc/samba/smb.conf 
sudo sed -i "5 s/.realm.*/realm = ${domainname^^}/" /etc/samba/smb.conf 
...
echo
echo -e "\e[31mPress ENTER to reboot the VM.\e[0m"
echo
